...
:bulb: The distance to closest road variable may actually not be relevant in the process as this category would actually rather be used for, e.g., performing risk analysis. However, it may correlate with the fact that more observation close to roads may be possible and actually done. Let's thus apply our workflow but this time excluding the `dist_roads` variable.
